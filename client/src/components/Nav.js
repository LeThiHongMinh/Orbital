  ...
  return (
    <div className = {`fixed top-0 left-0 w-full bg-red-50  rounded-sm z-50  border-b border-n-6 lg:bg-n-8/90 lg:backdrop-blur-sm ${
        ...
        <nav className = {`${
            ...
            <div className=" relative text-black z-10 flex flex-col space-y-10  items-center justify-center m-auto lg:space-x-10  lg:flex-row lg:space-y-0">
            {navigation.map((item) => (
            <a key={item.label}
                href={item.href}
                onClick={handleClick}
                ...
                className={`${
                  ...
                  }  lg:font-semibold space-y-11  lg:text-black m-auto stroke-black block shadow-2xl relative font-palanquin font-bold lg:static lg:flex lg:mx-auto lg:bg-n-8 leading-normal text-2xl transition-colors ${item.href === pathname.hash ? "z-2 lg:text-n-1 " : "lg-text-n-1/50 "}lg:leading-5 lg:hover:text-n-1 xl:px-12` }>
                    {item.label}
                </a>
      ...
      <Hamburger
          className="ml-auto bg-red-600 hover:bg-red-400 lg:hidden space-y-10 "
          px="px-3"
